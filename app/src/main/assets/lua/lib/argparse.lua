...
function parse_args(words, keywords, reverse_keywords)
    ...
    for word_index, word in ipairs(words) do
        ...
        for ki, keyword in ipairs(reverse_keywords) do
            if keyword == word then
                handle_match(start, words, word_index, args, reverse_args, extra_args, last_keyword, keyword)
                last_keyword = nil
                handled = true
                break
            end
        ...
        for ki, keyword in ipairs(keywords) do
            if keyword == word then
                handle_match(start, words, word_index, args, reverse_args, extra_args, last_keyword, nil)
    ...
    handle_match(start, words, #words + 1, args, reverse_args, extra_args, last_keyword, nil)
    return args, extra_args, reverse_args
end

function handle_match(start, words, word_index, args, reverse_args, extra_args, last_keyword, reverse_keyword)
    if start == -1 then
        if reverse_keyword ~= nil then
            insert_match(0, words, word_index, reverse_args, reverse_keyword)
        else
        ...
        if reverse_keyword ~= nil then
            insert_match(start, words, word_index, reverse_args, reverse_keyword)
        end
    end
end
